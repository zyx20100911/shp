        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <div v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                <dl>
                  <dt class="title">{{ spuSaleAttr.saleAttrName }}</dt>
                  <div v-for="(val,index) in spuSaleAttr.spuSaleAttrValueList" :key="val.id">
                    <dd changepirce="0" :class="{'active':val.isChecked ==='1'}"
                        @click="isActive(val,spuSaleAttr.spuSaleAttrValueList)">{{ val.saleAttrValueName }}
                    </dd>
                  </div>
                </dl>
              </div>
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="add(skuInfo.id)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...mapState({
      price: state => state.detail.DetailInfo.price,
      valuesSkuJson: state => state.detail.DetailInfo.valuesSkuJson
    }),
    ...mapGetters({
  ...
  methods: {
    ...
    add(skuId) {
      reqAddToCart({skuId, skuNum: this.skuNum}).then(res => {
        if (res.code === 200) {
          sessionStorage.setItem('skuInfo', JSON.stringify(this.skuInfo)) //本地会话存储购物车数据
